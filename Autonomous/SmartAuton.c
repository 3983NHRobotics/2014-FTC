...
void turnDeg(const string dir, int deg, int power) {
	//debug("Turning %d" + dir + " for %d" + deg + " degrees at %d" + power + " percent power");
	StringFormat(msg, "Turning %05d", deg);
	debug(msg);
	...
	if (dir == "left") {
		...
		StringFormat(msg, "Got invalid direction: ", dir); //rewrite debug.h to incorporate the stringformat in it
		debug(msg);
	}
...
void turnTime(const string dir, int time, int power) {
	StringFormat(msg, "Turning %05d", time/1000);
	debug(msg);
	...
	if (dir == "left") {
		...
		StringFormat(msg, "Got invalid direction: ", dir);
		debug(msg);
	}
}
